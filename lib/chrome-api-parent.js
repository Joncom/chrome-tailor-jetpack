...
function setup(options) {
  ...
  // TODO: Rename to chrome.helper.resizePopup
  target.port.on("resize", function(data) {
    console.log('Resizing panel to width=' + data.width + ', height=' + data.height);
    target.resize(data.width, data.height);
  });

  target.port.on("chrome.helper.setProxyCredentials", function(data) {
    var url = 'moz-proxy://' + data.host + ':' + data.port;
    var store = function() {
    ...
    require("sdk/passwords").search({
      ...
      onComplete: function onComplete(credentials) {
        // Removing exist credentials before storing
        if(credentials.length > 0) {
          var options = credentials[0];
          options.onComplete = function() {
            store();
          }
          require("sdk/passwords").remove(options);
        } else {
          store();
        }
      }
  ...
  target.port.on("chrome.browserAction.setIcon", function(path) {
    emit(emitter, "chrome.browserAction.setIcon", path);
  });
  ...
  target.port.on("chrome.proxy.settings.clear", function(callID) {
    ...
    target.port.emit("chrome.proxy.settings.clear::done", callID);
  });

  target.port.on("chrome.helper.enableProxyAutoLogin", function() {
    service.set("signon.autologin.proxy", true);
  });
  ...
  target.port.on("chrome.runtime.onMessage", function(data) {
    console.log("chrome.runtime.onMessage event fired...");
    emit(emitter, "chrome.runtime.onMessage", data);
  });
  on(emitter, "chrome.runtime.onMessage", function(data) {
    target.port.emit("chrome.runtime.onMessage", data);
  });
  target.port.on("chrome.runtime.onMessage::response", function(data) {
    emit(emitter, "chrome.runtime.onMessage::response", data);
  });
  on(emitter, "chrome.runtime.onMessage::response", function(data) {
    target.port.emit("chrome.runtime.onMessage::response", data);
  });
  ...
  target.port.on("storage:local:get", function(data) {
    ...
    var keysIsArray = Object.prototype.toString.call(keys) === '[object Array]';
    if (typeof keys == "object" && !keysIsArray) {
      defaults = keys;
